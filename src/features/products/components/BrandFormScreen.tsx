...
export default function BrandFormScreen({ id, mode }: { id?: string; mode: "create" | "edit" }) {
  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safeArea}><AppStatusBar /><KeyboardAwareScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" style={styles.flex}>
    <View style={styles.header}><TouchableOpacity hitSlop={12} onPress={back} style={styles.iconButton}><Ionicons name="chevron-back" size={19} color={Colors.primary} /></TouchableOpacity><Text style={styles.title}>{title}</Text><View style={styles.spacer} /></View>
